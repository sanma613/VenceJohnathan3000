...
public:
    ...
    virtual bool canBoop(PieceType placedPiece) const = 0;
...
public:
    ...
    bool canBoop(PieceType placedPiece) const override;
...
// Clase principal del juego Boop (Singleton)
class Boop
{
private:
    static Boop *instance;

    // Constructor privado para singleton
    Boop();

    // Eliminar constructor de copia y operador de asignación
    Boop(const Boop &) = delete;
    Boop &operator=(const Boop &) = delete;
...
public:
    ...
    static Boop *getInstance();

    // Método para resetear el juego (útil para backtracking)
    void reset();
...
inline bool Gatito::canBoop(PieceType placedPiece) const
{
    (void)placedPiece;
    return true;
}
...
inline bool Gato::canBoop(PieceType placedPiece) const
{
    if (placedPiece == PieceType::GATITO)
    {
...
inline vector<Piece *> Board::boopPieces(int placedRow, int placedCol, Piece *placedPiece)
{
    ...
    for (const auto &pos : adjacentPositions)
    {
        ...
        if (piece && piece->canBoop(placedPiece->piece_type))
        {
...
// Note: Boop::getInstance() y Boop::reset() deben implementarse en BoopGame.cpp
// junto con la inicialización de Boop::instance
